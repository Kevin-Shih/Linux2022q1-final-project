...
node_t *list_insert(list_t *ll, unsigned long int tid)
{
    ...
    tmp->runqueue.head = tmp->runqueue.tail = NULL;
    tmp->runqueue.uthread_count = 0;
    tmp->tid = tid;
    tmp->next = NULL;
    tmp->ret_val = NULL;
    if (!ll->head) {
 ...
 * @return Pointer to new node on success, NULL on failure
 */
uthread_node_t *uthread_list_insert(uthread_list_t *ll)
{
    uthread_node_t *tmp;
    if (posix_memalign((void **) &tmp, 8, sizeof(uthread_node_t))) {
        perror("ll alloc");
        return NULL;
    }
    tmp->next = NULL;
    ...
    ll->uthread_count++;
    syscall(SYS_futex, &(ll->uthread_count), FUTEX_WAKE, 1, NULL, NULL, 0);
    //printf("wake: %lu\n", (unsigned long)&(ll->uthread_count));
    return tmp;
}
...
static uthread_list_t uthread_list;
...
static void init()
{
    ...
    node->fa = NULL;
    node->runqueue.uthread_count=UINT64_MAX;
    atexit(cleanup);
}
...
node_t* find_min_runqueue(void){
    node_t *current = tid_list.head->next;
    node_t *min = tid_list.head;
	while(current){
        min = (current->runqueue.uthread_count) < (min->runqueue.uthread_count) ? current : min;
...
int uthread_create(void *routine, void *arg)
{
    ...
    //分到目前最少的 local run queue
    node_t *min = find_min_runqueue();
    uthread_node_t *node = uthread_list_insert(&min->runqueue);
    if (!node) {
        printf("Thread address not found\n");
        spin_release(&global_lock);
        return -1;
    }
    node->current_kthread = min->tid;
...
static void kthread_try_run(void *arg){
    ...
    while(1){
        if(__atomic_load_n(&runqueue->uthread_count, __ATOMIC_RELAXED) != 0){
            ...
            //syscall(SYS_futex, &(runqueue->uthread_count), FUTEX_WAIT, 0, NULL, NULL, 0);
        }
    }
}
...
int kthread_create(thread_t *t)
{
    ...
    fa->f = kthread_try_run;
    fa->arg = (void *)&node->runqueue;
...
int main(int argc, char *argv[]){
    ...
    thread_t kthread[thread_count];
    for(size_t i=0; i < thread_count; i++)
        kthread_create(&kthread[i]);
    //kthread sould try poll uthread(*fa) from run queue and execute it;
    ...
    while(1){
        ...
        uthread_create(handle_connection, (void*)cconn);
    }
    close(sconn->fd);
    return 0;
}